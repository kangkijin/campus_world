	...
	$tableTr.find("tbody").append(
					'<tr id="budgetTr_'+trIndex+'">'
						+'<td>'
							...
							+'<select class="form-control input-sm wd_p80 dp_inline" name="nbiBudgetCd" id="nbiBudgetCd" title="예산 대분류" onchange="fnBiBudgetCdSelChg(this, \'main\');">'
						...
						+'<td>'
							+'<div class="">'
								+'<select class="form-control input-sm wd_p80 dp_inline" name="nbiBudgetSubCd" id="nbiBudgetSubCd" title="예산 항" onchange="fnBiBudgetCdSelChg(this, \'sub\');">'
									+'<option value="">예산구분을 선택해주세요.</option>'
								+'</select>'
							+'</div>'
						+'</td>'
						+'<td>'
							+'<div class="">'
								+'<select class="form-control input-sm wd_p80 dp_inline" name="nbiBudgetThirdCd" id="nbiBudgetThirdCd" title="예산 목" onchange="fnBiBudgetCdSelChg(this, \'third\');">'
									+'<option value="">예산구분을 선택해주세요.</option>'
								+'</select>'
							+'</div>'
						+'</td>'
...
function addSubBudget(_obj,_trCnt) {
	...
	console.log($(_obj).parent('div').parent('td').parent('tr').children('td:eq(0)').children('input[name=budgetTrCnt]').val());
...
//					alert("여러개의 검색결과가 존재 합니다. 한개의 부서/학과를 선택해 주세요.");
					...
					fnGoOpenSuDeptListPopup(searchType,idx,_ncrDeptDiv);
...
**********************************************************
 **/
function fnGoOpenSuDeptListPopup(_searchType, idx, deptDiv){
	var deptNm = "";
	if(_searchType == "npiPartiDept"){
		deptNm = $("#npiPartiDeptNm_" + idx).val();
	}else{
	...
	fnOpenSuDeptListPopup(_searchType, deptNm, idx, deptDiv);
